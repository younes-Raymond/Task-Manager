...
app.use('/mail-track', expressApp({
  baseUrl: `http://localhost:${PORT}/mail-track`, // Adjust the base URL as needed
  jwtSecret: 'secret',
  getData: data => ({ ...data, hello: 'world' }),
  onBlankImageView: data => {
    // Handle the event when an email is opened
    console.log('Email opened:', data);
  },
  onLinkClick: async data => {
    // Handle the event when a link in the email is clicked
    try {
      const response = await axios.post('https://ajial.onrender.com/api/v1/TrackLinkClick', {
        email: data.recipient, // Use the recipient email or include it in the data when sending emails
        link: data.link,
        // Add other relevant data
      });

      console.log('Link click tracked:', response.data);
    } catch (error) {
...
const sendTrackedEmail = async (to, subject, html, attachments = [], trackingOptions = {}) => {
  try {
    ...
    const mergedTrackingOptions = { ...trackingOptions };

    await sendMail(mergedTrackingOptions, transporter, mailOptions);

    console.log(`Message sent to ${to}: Email sent and tracked successfully.`);
  } catch (error) {
    console.error(`Error sending and tracking email to ${to}:`, error);
  }
};

// Your sample HTML content
const sampleHtml = `
...
// Your sample attachments
const sampleAttachments = [
  {
    filename: 'cover_letter.txt',
    content: 'Dear Hiring Manager, ... (cover letter content) ...',
  },
  {
    filename: 'resume.pdf',
...
// Your sample tracking options
const sampleTrackingOptions = {
  // Add other tracking options if needed
};

// Launch the email campaign
const launchEmailCampaign = async () => {
  try {
    const emailList = ['youneshero435@gmail.com', 'another@example.com', /* Add more emails as needed */];

    for (const email of emailList) {
      await sendTrackedEmail(email, 'Job Application', sampleHtml, sampleAttachments, sampleTrackingOptions);
    }

    console.log('Email campaign launched successfully.');
  } catch (error) {
    console.error('Error launching email campaign:', error.message);
  }
};

// Call the function to launch the email campaign
// launchEmailCampaign();
